...
/*int tcp_connect(unsigned short port, char *addr)
{
    ...
    s.sin_addr.s_addr = BigLittleSwap32(addr);
    s.sin_port = BigLittleSwap16(port);
    // set to non-blocking
...
int tcp_listen(unsigned short port)
{
    ...
    s.sin_family = AF_INET;
    s.sin_addr.s_addr = BigLittleSwap32(INADDR_ANY);
    s.sin_port = BigLittleSwap16(port);
...
int tcp_read(int fd, void *buffer, int nbytes, struct sockaddr *Addr)
{
    ...
    if(n>0)
    {
        ...
        else
        {
            ...
            fprintf(stderr, "Port:%d\n",BigLittleSwap16(((struct sockaddr_in *)Addr)->sin_port));
        }
    }
                ...
                if(!sched[i].rtp_session->pause)
                {
                    ...
                    if((sched[i].rtp_session->hndRtp)&&(s32FindNal))
                    {
                        ...
                        if(sched[i].BeginFrame== 1)
                        {
							...
							//if (pfile)
				            //fwrite((void *)ringinfo.buffer, 1,ringinfo.size, pfile);
                        }
                    }
                }
            }

...
int RTSP_full_msg_rcvd(RTSP_buffer *rtsp, int *hdr_len, int *body_len)
{
    ...
    if (rtsp->in_buffer[0] == '$')
    {
        ...
        /*转化为主机字节序，因为长度是网络字节序*/
        if ( (bl = __bswap_constant_16(*intlvd_len)) <= rtsp->in_size)
        {
...
//    if ( (pcnt = sscanf(pRtsp->in_buffer, " %31s %255s %31s\n%15s", method, object, ver, hdr, &seq)) != 5){
	if ( (pcnt = sscanf(pRtsp->in_buffer, " %31s %255s %31s\n%15s", method, object, ver, hdr)) != 4){//读进与指定格式相符的数据
		printf("========\n%s\n==========\n",pRtsp->in_buffer);
		printf("%s ",method); 
		printf("%s ",object);
		printf("%s ",ver);
...
*/
//===========加
	if ((p = strstr(pRtsp->in_buffer, "CSeq")) == NULL) {//返回字符串第一次出现的地址
		return ERR_GENERIC;
	}else {
		if(sscanf(p,"%254s %d",trash,&seq)!=2){//读出数据
			return ERR_GENERIC;
		}
	}
//==========
...
*/
	...
	strcat(pDescr, "\r\n");
	strcat(pDescr, "s=H.264 Video, streamed by the  Media Server\r\n");//(session name)
	strcat(pDescr, "i=test.h264\r\n");//session的信息
   	strcat(pDescr, "t=0 0\r\n");//时间信息，分别表示开始的时间和结束的时间，一般在流媒体的直播的时移中见的比较多	
   	strcat(pDescr, "a=tool: Streaming Media v2018.11.30\r\n");//创建任务描述的工具的名称及版本号	
	strcat(pDescr, "a=type:broadcast\r\n");//会议类型
	strcat(pDescr, "a=control:*\r\n");
	//strcat(pDescr, "a=range:npt=0-\r\n");//用来表示媒体流的长度
	//strcat(pDescr, "a=x-qt-text-nam:H.264 Video, streamed by the Gree Media Server\r\n");	
...
//	trace_point();
    ...
    /*序列号*/
    if ((p = strstr(pRtsp->in_buffer, HDR_CSEQ)) == NULL)//返回出现CSeq位置的地址
    {
    	fprintf(stderr, "Error %s,%i\n", __FILE__, __LINE__);
        send_reply(400, 0, pRtsp);/* Bad Request */
...
//				sprintf(s8Str + strlen(s8Str), "RTP/AVP;multicast;ttl=%d;destination=%s;port=", (int)DEFAULT_TTL, descr->multicast);
			...
			sprintf(s8Str + strlen(s8Str), "%ld-%ld"RTSP_EL, server_port, server_port+1);
			break;
...
*/
                ...
                case RTSP_ID_OPTIONS:
					//printf("Run in this !!! Time = %d\n",ts_current);
					if (RTSP_options(pRtspBuf) == ERR_NOERROR)
                    {
                    	//pRtspSess->cur_state = INIT_STATE;         //状态不变
                    	printf("options again!!!\n");
                    }
...
**
**************************************************************************************************/
void base64_encode2(unsigned char *in, const int in_len, char *out, int out_len)
{
...
void UpdateSps(unsigned char *data,int len)
{
    ...
    //printf("Jump in UpdateSps psp.base64profileid=%s,psp.base64sps=%s\n",psp.base64profileid,psp.base64sps);
...
void UpdatePps(unsigned char *data,int len)
{
	...
	//printf("Jump in UpdatePps psp.base64pps=%s\n\n",psp.base64pps);
